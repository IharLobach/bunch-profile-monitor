...
class TransmissionElement():
    ...
    def prepend_01_point(self):
            ...
            self.TransmissionCoefs0 = np.concatenate((np.array([1]),self.TransmissionCoefs0))
    ...
    def resample(self,freqs):
        ...
        if freqs[0]<self.freqs0[0]:
            if self.extrapolate_left:
                ...
                raise ValueError(
            """New frequency array's lower limit is below the minimum frequency in the network analyzer file.
Adjust the new frequency array, or use extrapolation by setting attribute extrapolate_left of this TransmissionElement to True.""")
        if freqs[-1]>self.freqs0[-1]:
            if self.extrapolate_right:
                right = self.TransmissionCoefs0[-1]
            else:
                raise ValueError(
            """New frequency array's upper limit is above the maximum frequency in the network analyzer file.
Adjust the new frequency array, or use extrapolation by setting attribute extrapolate_right of this TransmissionElement to True.""")
        self.TransmissionCoefs = np.interp(freqs,self.freqs0,self.TransmissionCoefs0,left = left, right = right)
        self.freqs = freqs
...
class Amplifier(TransmissionElement):
    ...
    extrapolate_left=False,extrapolate_right=False,prepend_01_point=False):
        ...
        self.G = np.exp(1j * np.radians(self.phase_shifts.iloc[:, 1].values)) * np.power(10, self.dBs.iloc[:, 1].values / 20)
        self.TransmissionCoefs0 = self.G * np.exp(2j * np.pi * self.T_amplifier_ns * self.freqs0)
        if prepend_01_point:
...
class Cable(TransmissionElement):
    def __init__(self,phase_shift_short_filename, dB_short_filename,phase_shift_open_filename, dB_open_filename,
     T_cable_ns,T_cor_ns,extrapolate_left=False,extrapolate_right=False,prepend_01_point=False):
        super().__init__(extrapolate_left,extrapolate_right)
        self.T_cable_ns = T_cable_ns
        self.T_cor_ns = T_cor_ns
        self.phase_shifts_short = pd.read_csv(phase_shift_short_filename, skiprows=2)
        self.dBs_short = pd.read_csv(dB_short_filename, skiprows=2)
        self.phase_shifts_open = pd.read_csv(phase_shift_open_filename, skiprows=2)
        ...
        if sum(freqs_open-freqs_short)!=0:
            raise ValueError("Frequancy arrays for short and open cable measurements do not match.")
        self.freqs0 = freqs_open
        self.G_short = -np.exp(1j*np.radians(self.phase_shifts_short.iloc[:,1].values))*np.power(10,self.dBs_short.iloc[:,1].values/20)
        self.G_open = np.exp(1j*np.radians(self.phase_shifts_open.iloc[:,1].values))*np.power(10,self.dBs_open.iloc[:,1].values/20)
        self.TransmissionCoefs_short = self.G_short*np.exp(2j*np.pi*self.T_cable_ns*self.freqs0)
        self.TransmissionCoefs_open = self.G_open*np.exp(2j*np.pi*self.T_cable_ns*self.freqs0)
        self.TransmissionCoefs0 = np.sqrt((self.TransmissionCoefs_short+self.TransmissionCoefs_open)/2*np.exp(2j*np.pi*self.T_cor_ns*self.freqs0))
        if prepend_01_point:
            self.prepend_01_point()
        # initializing self.freqs and self.TransmissionCoefs:
        self.resample(self.freqs0)
...
class CableModel:
    ...
    def calc_attenuation(self,f):
        ...
        return self.dB_to_linear(-(1. + 1.j) * self.k1 * self.z * np.sqrt(f) - self.k2 * self.z * f)
...
if __name__=="__main__":
    freq_cutoff = 2#GHz
    amplifier = Amplifier("TRACE40.csv","TRACE41.csv",2.7,True,True,True)
    cable = Cable("TRACE15.csv","TRACE16.csv","TRACE20.csv","TRACE21.csv",2*0.229952e3,0.26,True,True,True)
    cable_model = HeliaxCableHalfInch(3)
    freqs = np.linspace(0,freq_cutoff,1e3)
    transfer_line = SignalTransferLine([cable,amplifier,cable_model],freqs=freqs)
    True
